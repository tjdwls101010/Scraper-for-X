---
name: X retrieval
description: Read X/Twitter via the agentic-x CLI — your home feed, a profile's tweets or replies, a tweet and its thread, search, or the social graph (following/followers/retweeters) — and chain those results to answer multi-hop questions. Use whenever the user wants something off X/Twitter, however they phrase it: "what has <person> been tweeting", "check my timeline", "who retweeted this", "what is <person>'s circle talking about", "search X for <topic>", "look up <name> on Twitter". Also use when the user hands over an x.com or twitter.com URL and wants its contents. NOT for developing or testing the agentic-x package itself (that is ordinary repo work), and not for any other social network.
allowed-tools: Bash(agentic-x:*), Bash(uv:*), Bash(pipx:*), Bash(curl:*), Read
---

# X retrieval

`agentic-x` gives you fast, structured retrieval. **You supply the navigation.** The CLI is deliberately a set of single-target primitives with no `crawl` command — deciding which handle to follow next is your job, and it is the whole reason this skill exists.
...
A repo checkout and the installed CLI are **different things**: `PYTHONPATH=src python -m agentic_x.cli` can be a completely different version from whatever `agentic-x` on PATH resolves to. The catalog's version is the one that counts.
...
```bash
agentic-x status          # exit 0 = ready; exit 2 = needs login
```

If exit 2: **`agentic-x login` opens a real browser window and needs a human to log in.** You cannot complete it; ask the user, then re-check. The account must be a throwaway — see Ban risk.
...
```bash
agentic-x feed --limit 10 --output /tmp/x-feed.json
...
Run `agentic-x schema` for the full field list of both. Prefer that over assuming — it's generated from the code, so it can't drift the way a copy here would.

One field misleads if you skim: `captured_at` is when *you* scraped it, not when anything happened on X. Sorting or deduping by it produces an ordering that looks plausible and means nothing — `id` dedups, `created_at` orders.
...
So: **write output to a temp path, not into the repo**, and never `git add` it — a scrape committed to a repo outlives the question it was collected for. Delete intermediate files when you're done, and tell the user the rest can go. Quote named individuals only when the question actually needs the quote; a summary usually answers it with less exposure.
...
`agentic-x catalog` prints what each exit code *means*. This is what to *do* — and the theme is that most failures here are informative, not transient. **Retrying the same command is rarely the fix.**

**Exit 4 on `search`, `fetch --replies`, or `followers`.** Upgrade first (Step 1) — this is the failure X's rotations cause, and the fix ships as a release. If you are already current, the fix doesn't exist yet, and the task has to be re-planned rather than retried.

Re-plan it, because **only those three commands are affected** — everything else is unaffected by whatever broke. `tweet --replies` still gets a thread, `following` still works when `followers` doesn't, and `feed`/`fetch`/`retweeters` are untouched. Reach for an equivalent that isn't blocked before telling the user it can't be done.

`search` and `fetch --replies` also fall back to the browser on their own, if the `[browser]` extra is installed. **That fallback returns the first page only** — say so; a truncated result presented as a complete one is the worst outcome available here.

**Exit 4 on anything else** — query-id drift. `agentic-x doctor --refresh` re-anchors the ids from x.com without a browser and usually fixes it in one step.

**Exit 2 — session expired.** Routine; sessions die. Needs a human at a browser (`agentic-x login`). Note that X reports a soft-locked session as HTTP 200 with an empty body, not a clean 401 — so `status` inspects the response, and if it says expired while a browser looks logged in, trust `status`.
