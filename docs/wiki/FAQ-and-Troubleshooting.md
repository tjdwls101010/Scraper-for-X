...
Questions people actually ask about this tool, and the errors people actually hit. If your issue isn't here, check [open issues](https://github.com/tjdwls101010/Agentic-X/issues) before filing a new one.
...
**Likers: not available, and not coming.** X removed the likers list from its product. `/status/<id>/likes` redirects to the tweet itself, and the operation name appears in none of the JavaScript x.com serves (checked across all 685 chunks, 2026-07-20). There is no `likers` command because there is nothing to call. `agentic-x retweeters <id>` is the closest real signal.

**Quoters: use search.** `agentic-x search "quoted_tweet_id:<id>"` returns a tweet's quote-tweets. That is exactly what X's own /quotes tab issues, which is why there is no separate command.
...
- `agentic-x doctor` lets you check session health without guessing.
...
This is X's **soft-lock** behavior: a stale or flagged session doesn't always come back as a clean 401 — it can return HTTP 200 with an empty or malformed body instead, which looks identical to "logged out" from the outside. `agentic-x status`/`doctor` (and the pre-exit-4 probe inside `fetch`/`search`/`tweet`) both check for this specifically, so if `status` reports `expired` right after a login that seemed to succeed, take it at face value rather than assuming it's a false positive — it usually means the session actually degraded (a security challenge fired silently, the account got flagged, or something about the request context, like an IP change, triggered a soft-lock).

Fix is the same as any expired session:

```bash
agentic-x login --profile <name>
```
...
### Can I run `agentic-x fetch`/`search` in a loop or cron job?
...
### `agentic-x setup` fails / can't download the browser

`agentic-x setup` provisions Chromium into this tool's own isolated `PLAYWRIGHT_BROWSERS_PATH` (never a browser install any other tool manages), by shelling out to `scrapling`'s own install mechanism. Failures here are almost always:
...
Open an issue at <https://github.com/tjdwls101010/Agentic-X/issues>. Useful things to include:

- Your OS and Python version, and the `agentic-x --version` output.
