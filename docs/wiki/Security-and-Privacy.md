...
`agentic-x login` opens a headed stealth browser, you log in by hand, and the tool harvests the resulting session — `auth_token`, `ct0`, the browser's user-agent, plus the query-ids/features it read off the page — into a small JSON file, `session.json`. `agentic-x login --cookies <file>` (or `XScraper.from_cookies`/`from_cookie_file`) does the same thing without a browser, by importing an already-established session's cookies.
...
**Where it lives:** `platformdirs.user_data_dir("agentic-x")/profiles/<profile>/session.json` (macOS: `~/Library/Application Support/agentic-x/profiles/default/session.json`). Override with `--profile-dir`/`XScraper(profile_dir=...)` or the `SFX_PROFILE_DIR` env var (see [Configuration](Configuration.md)).
...
`agentic-x login --cookies <file>` and `XScraper.from_cookie_file`/`from_cookies` exist so you can carry over a session you already established in a real browser, without ever running this tool's browser. `auth.py` treats that input as untrusted until proven otherwise:
...
- **Run from the same network/IP where the session was established.** Prefer running `agentic-x` from the same network/egress IP as wherever you ran `agentic-x login`, or wherever you originally exported cookies from. An abrupt IP or client change against an existing session — especially pairing a cookie-imported session with a datacenter/VPN IP — is exactly the kind of signal X's abuse systems weight, and it can soft-lock the session (X returns HTTP 200 with an empty/limited timeline instead of a clean 401) even without an outright ban. `agentic-x status`/`doctor` and the retrieval layer's pre-exit-4 viewer-lookup probe exist partly to detect this state and map it to exit code 2 (`agentic-x login`) rather than a misleading "empty result" or "parse drift" signal.
...
Open a GitHub issue on the repo (`tjdwls101010/Agentic-X`).
